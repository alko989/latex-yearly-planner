...
func Annual(cfg config.Config, tpls []string) (page.Modules, error) {
	...
	return page.Modules{{
		...
		Body: map[string]interface{}{
			...
			"Extra": header.Items{header.NewTextItem("Projects").RefText("Projects Index")}.
...
func extra2(ctrc, sel1, sel2 bool, week *cal.Week, idxPage int) header.Items {
	...
	if idxPage > 0 {
		...
		items = append(items, header.NewCellItem("Projects").Refer("Projects Index"+suffix).Selected(sel2))
	} else {
		items = append(items, header.NewCellItem("Projects").Refer("Projects Index").Selected(sel2))
	}
