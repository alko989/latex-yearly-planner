...
func (p Notes) Breadcrumb(year, idx int) string {
	...
	return header.Items{
		...
		header.NewTextItem("Projects Index" + postfix).Ref(true),
	}.Table(true)
}
...
func (p Notes) HeadingMOS(page, pages int) string {
	...
	out += tex.Hypertarget(p.ref(page), "    ") + tex.ResizeBoxW(`\myLenHeaderResizeBox`, `Projects - Index`)
...
func (n Note) Breadcrumb() string {
	...
	return header.Items{
		...
		header.NewTextItem("Projects Index" + page),
...
func (n Note) PrevNext(notes int) header.Items {
	...
	if n.Number > 1 {
		items = append(items, header.NewTextItem("Project "+strconv.Itoa(n.Number-1)))
	}
...
func (n Note) HeadingMOS(page int) string {
	...
	return tex.Hypertarget(n.ref(), "") + tex.ResizeBoxW(`\myLenHeaderResizeBox`, `Project `+num+`\myDummyQ`)
}

func (n Note) ref() string {
	return "Project " + strconv.Itoa(n.Number)
}
